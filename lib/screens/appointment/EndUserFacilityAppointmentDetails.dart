...
class EndUserFacilityAppointmentDetails extends StatefulWidget {
  EndUserAppointmentPassingModel? model;

  EndUserFacilityAppointmentDetails({Key? key, this.model}) : super(key: key);

  @override
  State<EndUserFacilityAppointmentDetails> createState() => _EndUserFacilityAppointmentDetailsState();
}

class _EndUserFacilityAppointmentDetailsState extends State<EndUserFacilityAppointmentDetails> {
                            ...
                            if (_facilityAppointmentDetailModel!.paymentTransaction != null &&
                              ...
                              showModalBottomSheet(
                                  ...
                                  builder: (BuildContext ctx) {
                                    ...
                                    return _facilityTransactionBottomSheet(ctx, tempList);
                                  });
                            } else {
          ...
          children: [
            Column(
              children: [
                ...
                RatingBar.builder(
                  initialRating:
                      _facilityAppointmentDetailModel!.facilityBookingReviews != null ? _facilityAppointmentDetailModel!.facilityBookingReviews!.rating! : 0.0,
                  minRating: 0,
        ...
        isButtonVisible
            ? MyButton(
                ...
                onTap: () async {
                  ...
                  await Navigator.pushNamed(context, Constants.facilityEndUserCancelAppointment, arguments: endUserAppointmentPassingModel);
                },
              )
            : Container(),
        const SizedBox(
  ...
  PreferredSizeWidget appbarView() {
    return CustomAppBar(title: 'Appointment Details', onBack: () => Navigator.pop(context));
  }
      ...
      if (facilityAppointmentDetailModel.facilityBookingId != null) {
        setState(() {
          ...
          for (var i = 0; i < _facilityAppointmentDetailModel!.facilityBookingSlotDates!.length; i++) {
            ...
            bookingAmount = bookingAmount + _facilityAppointmentDetailModel!.facilityBookingSlotDates![i].bookingAmount!;
            debugPrint('Booking Amount -> $bookingAmount');
            // tempPaidAmount += _facilityAppointmentDetailModel!.facilityBookingSlotDates![i].amount ?? 0.00;
            // tempTotalAmount += _facilityAppointmentDetailModel!.facilityBookingSlotDates![i].totalAmount ?? 0.00;
            discountedAmount += _facilityAppointmentDetailModel!.facilityBookingSlotDates![i].discountAmount ?? 0.00;
            if (!_facilityAppointmentDetailModel!.facilityBookingSlotDates![i].isCancelled!) {
              tempPaidAmount += _facilityAppointmentDetailModel!.facilityBookingSlotDates![i].amount ?? 0.00;
              tempTotalAmount += _facilityAppointmentDetailModel!.facilityBookingSlotDates![i].totalAmount ?? 0.00;
              tempPayLater += _facilityAppointmentDetailModel!.facilityBookingSlotDates![i].totalAmount! -
                  (_facilityAppointmentDetailModel!.facilityBookingSlotDates![i].amount! +
                      _facilityAppointmentDetailModel!.facilityBookingSlotDates![i].discountAmount!);
            }
          }
          discountPer = facilityAppointmentDetailModel.facilityBookingSlotDates![0].discountPer ?? 0;
          ...
          if (payLaterAmount.isNegative) {
            payLaterAmount = 0.00;
            debugPrint("================ Pay Later Amount is Negative in EndUserFacilityAppointmentDetails ==============");
          }
          if (_facilityAppointmentDetailModel!.facilityBookingReviews != null) {
            if (_facilityAppointmentDetailModel!.facilityBookingReviews!.comment!.isEmpty) {
              feedback.text = '';
            } else {
              feedback.text = _facilityAppointmentDetailModel!.facilityBookingReviews!.comment!;
            }
          }
  ...
  Future callAddRating() async {
    ...
    if (_facilityAppointmentDetailModel!.facilityBookingReviews != null) {
      request['ReviewId'] = _facilityAppointmentDetailModel!.facilityBookingReviews!.reviewId.toString();
    }
    try {
      await _progressDialog.show();
      String response = '';
      if (_facilityAppointmentDetailModel!.facilityBookingReviews != null) {
        response = await Provider.of<AppointmentViewModel>(context, listen: false).facilityEditReviewRequest(request);
      } else {
        response = await Provider.of<AppointmentViewModel>(context, listen: false).facilityAddReviewRequest(request);
      }
            ...
            child: ListView.separated(
              ...
              itemBuilder: (BuildContext context, int index) {
                ...
                if (tempAmount != 0.00 && tempTotalAmount != 0.00) {
                  payLaterAmount = tempTotalAmount - tempAmount;
                  if (payLaterAmount.isNegative) {
                    payLaterAmount = 0.00;
                  }
                }
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Padding(
                        padding: const EdgeInsets.fromLTRB(0, 6, 16, 0),
                        child: Text(
                          'Amount : S\$ ${paymentTransaction[index].amount?.toStringAsFixed(2) ?? 0.00} ( Order fees )',
                          maxLines: 3,
                          style: TextStyle(fontSize: 14, color: ColorsUtils.greyText, fontWeight: FontWeight.w500),
                          overflow: TextOverflow.ellipsis,
                        ),
                      ...
                      Padding(
                        padding: const EdgeInsets.fromLTRB(0, 6, 16, 0),
                        child: Text(
                          'Coupon Code : ${_facilityAppointmentDetailModel?.referralCouponText ?? '-'}',
                          maxLines: 3,
                          style: TextStyle(fontSize: 14, color: ColorsUtils.greyText, fontWeight: FontWeight.w500),
                          overflow: TextOverflow.ellipsis,
                        ),
