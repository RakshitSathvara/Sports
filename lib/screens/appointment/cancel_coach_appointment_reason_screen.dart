...
class CancelCoachAppointmentReasonScreen extends StatefulWidget {
  CancelCoachAppointmentModel? cancelCoachAppointmentModel;

  CancelCoachAppointmentReasonScreen({Key? key, this.cancelCoachAppointmentModel}) : super(key: key);

  @override
  State<CancelCoachAppointmentReasonScreen> createState() => _CancelCoachAppointmentReasonScreenState();
}

class _CancelCoachAppointmentReasonScreenState extends State<CancelCoachAppointmentReasonScreen> {
  ...
  Future<void> verifyFacilityCancellationRequest() async {
    try {
      ...
      CoachCancelAppointmentVerificationResponseModel response =
          await Provider.of<AppointmentViewModel>(context, listen: false).verifyCoachCancelAppointmentCancel(request);
      await _progressDialog.hide();
      showLog('Response -> $response');
      if (response.coachCancelAppointmentSlotDtos!.isNotEmpty) {
        showLog('Name -> ${response.coachCancelAppointmentSlotDtos![0].refundedAmount}');
        for (var i = 0; i < widget.cancelCoachAppointmentModel!.bookingSlotList!.length; i++) {
          for (var j = 0; j < response.coachCancelAppointmentSlotDtos!.length; j++) {
            if (widget.cancelCoachAppointmentModel!.bookingSlotList![i].coachBatchSetupDaySlotMapId == response.coachCancelAppointmentSlotDtos![j].slotMapId) {
              setState(() {
                widget.cancelCoachAppointmentModel!.bookingSlotList![i].refundedAmount = response.coachCancelAppointmentSlotDtos![j].refundedAmount;
              });
            }
          }
        }
        widget.cancelCoachAppointmentModel!.selectedReasonId = reasonValue;
        widget.cancelCoachAppointmentModel!.selectedReason = selectedReasonStr;
        widget.cancelCoachAppointmentModel!.otherText = _otherReason.text.toString().trim();
        showSnackBarColor('Eligibility to receive refund checked', context, false);
        Navigator.of(context).pushNamed(Constants.coachVerifyCancelApppointment, arguments: widget.cancelCoachAppointmentModel);
      }
    } on CommonException catch (error) {
      await _progressDialog.hide();
      debugPrint(error.toString());
  ...
  Future<void> facilityCancelRequest() async {
    ...
    try {
      await _progressDialog.show();
      Map<String, dynamic> request = {};
      // request['SubActivityId'] = widget.cancelCoachAppointmentModel!.coachAppointmentDetailResponseModel!.subActivityId;
      request['UserId'] = OQDOApplication.instance.endUserID;
      request['CancelReasonId'] = reasonValue;
      request['OtherReason'] = _otherReason.text.toString().trim();
      ...
      await _progressDialog.hide();
      if (list.isNotEmpty) {
        showSnackBarColor('Appointment Cancelled', context, false);
