...
class CoachAppointmentsScreen extends StatefulWidget {
  DateTime? dateTime;
  CoachAppointmentsScreen({Key? key, this.dateTime}) : super(key: key);
...
class _CoachAppointmentsScreenState extends State<CoachAppointmentsScreen> {
  ...
  List<CoachAppointmentsResponseModel> _endUserAppointmentList = [];
  List<CoachAppointmentsResponseModel> _endUserAppointmentResponseList = [];
  String monthStr = '';
  DateTime? firstDate;
  DateTime? lastDate;
  ...
  Widget singleDayEventView(int index, List<CoachAppointmentListingSlotDtos> data, CoachAppointmentsResponseModel endUserAppointmentResponseList) {
    if (data.isNotEmpty) {
      return SizedBox(
        height: 120.0,
        child: GestureDetector(
          onTap: () async {
            if (data.isNotEmpty) {
              await Navigator.pushNamed(context, Constants.coachAppointmentDetailScreen, arguments: data[index].bookingId.toString());
            }
          },
          child: Container(
            padding: const EdgeInsets.all(2.0),
              ...
              showModalBottomSheet(
                ...
                if (value != null) {
                  ...
                  setState(() {
                    ...
                    monthChangeStr();
                    var item = _endUserAppointmentResponseList
    ...
    try {
      ...
      if (endUserAppointmentList.isNotEmpty) {
        setState(() {
          ...
          CoachAppointmentsResponseModel firstDateOfAppointment = _endUserAppointmentResponseList.first;
          CoachAppointmentsResponseModel lastDateOfAppointment = _endUserAppointmentResponseList.last;
  ...
  Future<void> getEndUserOtherAppointments(String passingDate) async {
    ...
    try {
      // await _progressDialog.show();
      List<CoachAppointmentsResponseModel> endUserAppointmentList = await Provider.of<AppointmentViewModel>(context, listen: false)
          .getCoachProviderAppointments(OQDOApplication.instance.coachID!, passingDate, convertDateTimeToString(kLastDay));
      if (!mounted) return;
      await _progressDialog.hide();
      if (endUserAppointmentList.isNotEmpty) {
        setState(() {
