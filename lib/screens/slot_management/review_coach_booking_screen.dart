...
class ReviewCoachBookingScreen extends StatefulWidget {
  CalendarViewModel? calendarViewModel;

  ReviewCoachBookingScreen({Key? key, this.calendarViewModel}) : super(key: key);

  @override
  State<ReviewCoachBookingScreen> createState() => _ReviewCoachBookingScreenState();
}

class _ReviewCoachBookingScreenState extends State<ReviewCoachBookingScreen> {
  late ProgressDialog _progressDialog;
  double totalAmount = 0.00;
  CoachBookingListModelResponse? _coachBookingListModelResponse;
                              ...
                              children: [
                                ...
                                Padding(
                                  ...
                                  child: CustomTextView(
                                    maxLine: 2,
                                    label:
                                        '${widget.calendarViewModel!.coachDetailsResponseModel!.name} - ${widget.calendarViewModel!.coachDetailsResponseModel!.coachName}',
                                ...
                                ListView.builder(
                                  ...
                                  itemCount: _coachBookingListModelResponse!.coachBookingFreezeSlots!.length,
              ...
              children: [
                Transform.scale(
                  ...
                  child: Checkbox(
                      ...
                      value: _coachBookingListModelResponse!.coachBookingFreezeSlots![index].isSlotSelected,
                      onChanged: (value) {
                        if (value!) {
                          setState(() {
                            _coachBookingListModelResponse!.coachBookingFreezeSlots![index].isSlotSelected = value;
                            totalAmountCalculation(value, _coachBookingListModelResponse!.coachBookingFreezeSlots![index].totalAmount ?? 0.00,
                                _coachBookingListModelResponse!.coachBookingFreezeSlots![index].amount ?? 0.00);
                          });
                        } else {
                          setState(() {
                            _coachBookingListModelResponse!.coachBookingFreezeSlots![index].isSlotSelected = value;
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      CustomTextView(
                        label:
                            '${convertDateToString(_coachBookingListModelResponse!.coachBookingFreezeSlots![index].bookingDate!).split(',')[1].split(' ')[2]} ${convertDateToString(_coachBookingListModelResponse!.coachBookingFreezeSlots![index].bookingDate!).split(',')[1].split(' ')[1]} - ${convertDateToString(_coachBookingListModelResponse!.coachBookingFreezeSlots![index].bookingDate!).split(',')[0]}',
                        maxLine: 2,
  ...
  bool isAnySlotSelected() {
    ...
    for (int i = 0; i < _coachBookingListModelResponse!.coachBookingFreezeSlots!.length; i++) {
      if (_coachBookingListModelResponse!.coachBookingFreezeSlots![i].isSlotSelected!) {
  ...
  Future<void> getFreezeBooking() async {
    try {
      CoachBookingListModelResponse response =
          await Provider.of<SlotManagementViewModel>(context, listen: false).getCoachFreezeBooking(widget.calendarViewModel!.coachFreezeId!);
      if (response.coachBookingFreezeId != 0) {
        setState(() {
          _coachBookingListModelResponse = response;
          totalAmount = _coachBookingListModelResponse!.totalAmount!;
          payNowAmount = _coachBookingListModelResponse!.amount!;
      ...
      if (error.code == 400) {
        var responseData = json.decode(error.message);
        if (responseData.containsKey('ModelState')) {
          showSnackBarColor(error.message, context, true);
        } else {
          Map<String, dynamic> errorModel = json.decode(error.message);
  ...
  void calculateDiscount() {
    if (isCouponSelected && selectedCouponIntent.discount > 0) {
      ...
      payNowAmount = 0.0;
      payLaterAmount = 0.0;
      for (var slot in _coachBookingListModelResponse!.coachBookingFreezeSlots!) {
  ...
  Future<void> coachAppointmentBooking() async {
    ...
    try {
      ...
      request['TranineeAddressId'] = widget.calendarViewModel!.selectedAddressId;
      request['CoachBatchSetupDetailId'] = widget.calendarViewModel!.coachDetailsResponseModel!.coachBatchSetupDetailId;
      request['CoachBookingFreezeId'] = widget.calendarViewModel!.coachFreezeId;
      request['TotalAmt'] = !isCouponSelected ? payNowAmount : (payNowAmount - discountedAmount);
      List<Map> facilityBookingSlotDateDtos = [];
      for (int i = 0; i < _coachBookingListModelResponse!.coachBookingFreezeSlots!.length; i++) {
        Map map = {};
        if (_coachBookingListModelResponse!.coachBookingFreezeSlots![i].isSlotSelected!) {
          map['BookingDate'] = _coachBookingListModelResponse!.coachBookingFreezeSlots![i].bookingDate;
          map['CoachBatchSetupDaySlotMapId'] = _coachBookingListModelResponse!.coachBookingFreezeSlots![i].coachBatchSetupDaySlotMapId;
          facilityBookingSlotDateDtos.add(map);
        }
      }

      request['CoachId'] = widget.calendarViewModel!.coachDetailsResponseModel?.coachId;
      request['CoachBookingSlotDateDtos'] = facilityBookingSlotDateDtos;
      request['EndUserReferralActivityId'] = !isCouponSelected ? null : selectedCouponIntent.couponId;
      debugPrint('coachAppointmentBooking -> ${jsonEncode(request)}');
      CoachBookingResponse response = await Provider.of<SlotManagementViewModel>(context, listen: false).coachSlotBooking(request);
      await _progressDialog.hide();
      if (!mounted) return;
      bookingRemainingTimer!.cancel();
      isPaymentSheetVisible = true;
      setState(() {});
      makePayment(response.paymentSecret!, response.coachBookingId!);
    } on CommonException catch (error) {
      await _progressDialog.hide();
      if (!mounted) return;
      debugPrint(error.toString());
  ...
  Future<void> makePayment(String secret, int coachBookingId) async {
    isBack = false;
    ...
    try {
      ...
      displayPaymentSheet(secret, coachBookingId);
    } catch (err) {
  ...
  displayPaymentSheet(String secret, int coachBookingId) async {
    String paymentID = "";
    try {
      var paymentIntent = await Stripe.instance.retrievePaymentIntent(secret);
      paymentID = paymentIntent.id;
      await Stripe.instance.presentPaymentSheet().then((value) {
        validateFacilityPaymentActivity(paymentID, coachBookingId);
        // successBooking(paymentID, coachBookingId);
  ...
  Future<void> validateFacilityPaymentActivity(String paymentID, int coachBookingId) async {
    ...
    try {
      ...
      request['FacilityBookingId'] = 0;
      request['CoachBookingId'] = coachBookingId;
      request['FacilityBookingSlotId'] = 0;
      request['CoachBookingSlotId'] = 0;
      request['CoachBookingFreezeId'] = widget.calendarViewModel!.coachFreezeId;
      request['FacilityBookingFreezeId'] = 0;
      ...
      var response = await Provider.of<SlotManagementViewModel>(context, listen: false).validateCoachPaymentActivity(request);
      await _progressDialog.hide();
      if (!mounted) return;
      if (response) {
        successBooking(paymentID, coachBookingId);
  ...
  Future<void> successBooking(String paymentId, int coachBookingId) async {
    ...
    try {
      ...
      bool response = await Provider.of<SlotManagementViewModel>(context, listen: false).successCoachBooking(request);
      await _progressDialog.hide();
      if (!mounted) return;
      if (response) {
        showDialog(
                    ...
                    content: Column(
                      ...
                      children: [
                        ...
                        MyButton(
                          ...
                          radius: 15,
                          onTap: () async {
                            showSnackBarColor('Coach booked', context, false);
                            await Navigator.pushNamedAndRemoveUntil(context, Constants.APPPAGES, Helper.of(context).predicate, arguments: 0);
                          },
                        ),
                      ],
  ...
  Future<void> failedBooking(String paymentId, int coachBookingId) async {
    ...
    try {
      ...
      request['OrderId'] = paymentId;
      request['Amount'] = !isCouponSelected ? payNowAmount : (payNowAmount - discountedAmount);
      bool response = await Provider.of<SlotManagementViewModel>(context, listen: false).failedCoachBooking(request);
      await _progressDialog.hide();
      if (!mounted) return;
      if (response) {
        showDialog(
                    ...
                    content: Column(
                      ...
                      children: [
                        ...
                        MyButton(
                          ...
                          radius: 15,
                          onTap: () async {
                            showSnackBarColor('Coach Booking Failed', context, true);
                            await Navigator.pushNamedAndRemoveUntil(context, Constants.APPPAGES, Helper.of(context).predicate, arguments: 0);
                          },
                        ),
                      ],
