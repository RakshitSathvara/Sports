...
class FacilityCancelSlotManagementScreen extends StatefulWidget {
  CalendarViewModel? calendarViewModel;

  FacilityCancelSlotManagementScreen({Key? key, this.calendarViewModel}) : super(key: key);

  @override
  State<FacilityCancelSlotManagementScreen> createState() => _FacilityCancelSlotManagementScreenState();
}

class _FacilityCancelSlotManagementScreenState extends State<FacilityCancelSlotManagementScreen> {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      if (await NetworkConnectionInterceptor().isConnected()) {
        get21DaysFacilitySlots(
            _calendarViewModel.facilitySetupId, convertDateTimeToString(_calendarViewModel.selectedDateTime!), convertDateTimeToString(kLastDay));
      } else {
                        ...
                        Padding(
                          ...
                          child: MyButton(
                            ...
                            onTap: () async {
                              ...
                              cancelReasonViewModel.type = 'F';
                              cancelReasonViewModel.facilityBookingList = _facilitySlotBookingModelList;
  ...
  void showSlotsList() {
    ...
    if (selectedDayStr.compareTo(currentDate) == 0) {
      setState(() {
        ...
        int finalMinutes = int.parse(minutes) + 2;
        ...
        if (_singleFacilityDatesList.isNotEmpty) {
          if (_singleFacilityDatesList[0].listOfSlots!.isNotEmpty) {
            getFacilityAvalableSlotByDate(_calendarViewModel.facilitySetupId, convertDateTimeToString(_selectedDay!));
          }
        } else {
          get21DaysFacilitySlots(_calendarViewModel.facilitySetupId, convertDateTimeToString(_selectedDay!), convertDateTimeToString(kLastDay));
        }
      });
    } else {
      setState(() {
