...
import '../../model/coach_available_slots_by_date_response_model.dart';
import '../../utils/utilities.dart';

class CoachSlotSelectionScreen extends StatefulWidget {
  CalendarViewModel? calendarViewModel;

  CoachSlotSelectionScreen({Key? key, this.calendarViewModel}) : super(key: key);

  @override
  State<CoachSlotSelectionScreen> createState() => _CoachSlotSelectionScreenState();
}

class _CoachSlotSelectionScreenState extends State<CoachSlotSelectionScreen> {
  ...
  List<CoachSlotListModel> unFreezeCountList = [];
  String minPass = '';
  String secondPass = '';
  int freezedSlotCount = 0;
  int minutes = 0;
  int seconds = 0;
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      get21DaysFacilitySlots(
          _calendarViewModel.coachDetailsResponseModel!.coachBatchSetupId, convertDateTimeToString(kFirstDay), convertDateTimeToString(kLastDay));
    });
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                ...
                totalAmount > 0 && (freezedSlotCount >= _calendarViewModel.coachDetailsResponseModel!.minumumSlot!)
                    ? Padding(
  ...
  String checkForAvailableSlots(int? facilitySetupDaySlotMapId, int index) {
    List<CoachAvailableSlotsByDateResponseModel> availableSlotsModelList =
        _availableSlotsByDateResponseModelList.where((element) => element.coachBatchSetupDaySlotMapId == facilitySetupDaySlotMapId!).toList();
  ...
  String checkForFreezeFacilityBookingId() {
    String coachBookingFreezeId = '';
    for (int i = 0; i < _facilitySlotBookingModelList.length; i++) {
      for (int j = 0; j < _facilitySlotBookingModelList[i].listOfSlots!.length; j++) {
        if (_facilitySlotBookingModelList[i].listOfSlots![j].coachBookingFreezeId > 0) {
          debugPrint('facilityFreezeBookingId -> $coachBookingFreezeId');
          coachBookingFreezeId = _facilitySlotBookingModelList[i].listOfSlots![j].coachBookingFreezeId.toString();
          break;
        }
      }
    }
    debugPrint('after loop -> $coachBookingFreezeId');
    return coachBookingFreezeId;
  }
      ...
      setState(() {
        ...
        if (_singleFacilityDatesList.isNotEmpty) {
          if (_singleFacilityDatesList[0].listOfSlots!.isNotEmpty) {
            getFacilityAvailableSlotByDate(_calendarViewModel.coachDetailsResponseModel!.coachBatchSetupId, convertDateTimeToString(_selectedDay!));
          }
        } else {
          get21DaysFacilitySlots(
              _calendarViewModel.coachDetailsResponseModel!.coachBatchSetupId, convertDateTimeToString(_selectedDay!), convertDateTimeToString(kLastDay));
        }
      });
    } else {
      setState(() {
  ...
  Future<void> freezeFacilityBookingCall(CoachSlotListModel slotList) async {
    // _progressDialog = ProgressDialog(context, type: ProgressDialogType.normal, isDismissible: false);
    // _progressDialog.style(message: "Please wait..");
    Map<String, dynamic> request = {};
    request['EndUserId'] = OQDOApplication.instance.endUserID.toString();
    request['CoachBatchSetupDetailId'] = _calendarViewModel.coachDetailsResponseModel!.coachBatchSetupDetailId.toString();
    request['IsSlotSelected'] = true;
    request['CoachBookingFreezeId'] = checkForFreezeFacilityBookingId() != ' ' ? checkForFreezeFacilityBookingId() : '';
    request['BookingDate'] = convertDateTimeToString(_selectedDay!);
    request['CoachBatchSetupDaySlotMapId'] = slotList.coachBatchSetupDaySlotMapId;
    debugPrint('request -> $request');
    try {
      await _progressDialog.show();
      FreezeCoachResponseModel response = await Provider.of<SlotManagementViewModel>(context, listen: false).freezeCoachBooking(request);
      _progressDialog.hide();
      if (response.coachBatchSetupDaySlotMapId! > 0) {
        if (slotList.coachBatchSetupDaySlotMapId == response.coachBatchSetupDaySlotMapId) {
          setState(() {
            slotList.coachBookingFreezeId = response.coachBookingFreezeId!;
            slotList.isSlotSelected = response.isSlotSelected ?? false;
            ...
            freezedSlotCount = freezedSlotCount + 1;
  ...
  Future<void> unFreezeFacilityBookingCall(CoachSlotListModel slotList, String type) async {
    Map<String, dynamic> request = {};
    request['EndUserId'] = OQDOApplication.instance.endUserID.toString();
    request['CoachBatchSetupDetailId'] = _calendarViewModel.coachDetailsResponseModel!.coachBatchSetupDetailId.toString();
    ...
    try {
      await _progressDialog.show();
      FreezeCoachResponseModel response = await Provider.of<SlotManagementViewModel>(context, listen: false).unFreezeCoachBooking(request);
      _progressDialog.hide();
      if (response.coachBatchSetupDaySlotMapId! > 0) {
        if (slotList.coachBatchSetupDaySlotMapId == response.coachBatchSetupDaySlotMapId) {
          setState(() {
          ...
          //     _calendarViewModel.coachDetailsResponseModel!.coachBatchSetupId, convertDateTimeToString(_selectedDay!));
        }
      }
    } on CommonException catch (error) {
      await _progressDialog.hide();
    ...
    try {
      if (unFreezeCountList.isNotEmpty) {
        ...
        for (int i = 0; i < unFreezeCountList.length; i++) {
          ...
          request['CoachBatchSetupDaySlotMapId'] = unFreezeCountList[i].coachBatchSetupDaySlotMapId;
          requestList.add(request);
        }
        bool responseList = await Provider.of<SlotManagementViewModel>(context, listen: false).unFreezeCoachBookingBulk(requestList);
        await _progressDialog.hide();
        if (responseList) {
  ...
  Future<void> getFreezeBooking() async {
    ...
    try {
      CoachBookingListModelResponse response =
          await Provider.of<SlotManagementViewModel>(context, listen: false).getCoachFreezeBooking(checkForFreezeFacilityBookingId());
      if (response.coachBookingFreezeId != 0) {
        await _progressDialog.hide();
        debugPrint('Id -> ${widget.calendarViewModel!.selectedAddressId}');
        _calendarViewModel.coachFreezeId = checkForFreezeFacilityBookingId();
        _calendarViewModel.remainingTime = '$minutes-$seconds';
        _calendarViewModel.bookingStartTime = bookingStartTime;
        Navigator.of(context).pushNamed(Constants.coachReviewBookingScreen, arguments: _calendarViewModel);
      }
    } on CommonException catch (error) {
      await _progressDialog.hide();
      debugPrint(error.toString());
