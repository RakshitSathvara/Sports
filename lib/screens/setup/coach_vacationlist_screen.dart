...
class CoachVacationListScreen extends StatefulWidget {
  const CoachVacationListScreen({super.key});

  @override
  State<CoachVacationListScreen> createState() => _CoachVacationListScreenState();
}

class _CoachVacationListScreenState extends State<CoachVacationListScreen> {
  late ProgressDialog _progressDialog;
  final List<CoachVacationResponseModel> _facilityVacationList = [];
  ...
  Future<void> getFacilityVacationCall() async {
    ...
    try {
      ...
      List<CoachVacationResponseModel> response =
          await Provider.of<ServiceProviderSetupViewModel>(context, listen: false).getCoachVacationCall(OQDOApplication.instance.coachID!);
  ...
  Future<void> deleteVacation(int? vacationId) async {
    ...
    try {
      ...
      String response = await Provider.of<ServiceProviderSetupViewModel>(context, listen: false).deleteCoachVacation(request);
      await _progressDialog.hide();
      if (response.isNotEmpty) {
        setState(() {
          showSnackBarColor('Vacation deleted', context, false);
          getFacilityVacationCall();
        });
      }
    } on CommonException catch (error) {
