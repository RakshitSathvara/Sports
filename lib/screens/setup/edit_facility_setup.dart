...
class EditFacilitySetupScreen extends StatefulWidget {
  GetFacilityByIdModel? getFacilityByIdModel;

  EditFacilitySetupScreen({Key? key, this.getFacilityByIdModel}) : super(key: key);

  @override
  State<EditFacilitySetupScreen> createState() => _EditFacilitySetupScreenState();
}

class _EditFacilitySetupScreenState extends State<EditFacilitySetupScreen> {
  ...
  void setFacilityData() async {
    titleController.text = widget.getFacilityByIdModel!.title!;
    subTitleController.text = widget.getFacilityByIdModel!.subTitle!;
    descriptionController.text = widget.getFacilityByIdModel!.description!;
    bookingType = widget.getFacilityByIdModel!.bookingType!;
    previousBookingType = widget.getFacilityByIdModel!.bookingType!;
    _isSameRatesForSlots = widget.getFacilityByIdModel!.isSameSlotRate!;
    if (_isSameRatesForSlots) {
      priceController.text = "${widget.getFacilityByIdModel?.ratePerHour ?? ""}";
      slotRateController.text = priceController.text;
    }
    previousPrice = widget.getFacilityByIdModel!.ratePerHour.toString();
    previousCapacity = widget.getFacilityByIdModel!.facilityCapacity!.toString();
    previousGroupSize = "${widget.getFacilityByIdModel?.maxGroupSize ?? ""}";
    _slotTimeInMins = widget.getFacilityByIdModel!.slotTimeMinute!;
    previousTimeSlot = widget.getFacilityByIdModel!.slotTimeMinute!;
    personController.text = widget.getFacilityByIdModel!.facilityCapacity!.toString();
    groupSizeController.text = bookingType == "G" ? "${widget.getFacilityByIdModel?.maxGroupSize ?? ""}" : "";
    String? slotTimeInHrs = widget.getFacilityByIdModel!.slotTimeHour;
    ...
    choosedinterest = widget.getFacilityByIdModel!.activityName;
    choosediActivity = widget.getFacilityByIdModel!.subActivityName;
    listingImgId = widget.getFacilityByIdModel!.listingPageImageId;
    listImagePath = widget.getFacilityByIdModel!.listingPageImage;
    selectedSubActivityID = widget.getFacilityByIdModel!.subActivityId.toString();
    ...
    personController.addListener(() {
      if (personController.text.toString().trim() != previousCapacity) {
  ...
  void getMultipleImages() async {
    for (int i = 0; i < widget.getFacilityByIdModel!.facilitySetupImages!.length; i++) {
      multipleImgsList.add(widget.getFacilityByIdModel!.facilitySetupImages![i].filePath!);
      facilityImgIds!.add(widget.getFacilityByIdModel!.facilitySetupImages![i].fileStorageId!);
    }
    setState(() {});
  }
                ...
                child: Column(
                  ...
                  children: [
                      ...
                      children: [
                        Align(
                          ...
                          child: GestureDetector(
                            onTap: () {
                              if (imagesList!.length > 2 || multipleImgsList!.length > 2) {
                        ...
                        isMultipleImgsEdited!
                            ...
                            : Row(
                                children: [
                                  Visibility(
                                    visible: multipleImgsList!.isNotEmpty,
                                    child: GestureDetector(
                                        onTap: () {
                                          facilityImgIds!.clear();
                                          multipleImgsList!.clear();
                                          setState(() {});
                                        },
                                        child: CustomTextView(label: 'Clear')),
                                  ),
                                        ...
                                        child: PageView.builder(
                                            ...
                                            itemBuilder: (context, index) {
                                              return Card(
                                                  ...
                                                  child: multipleImgsList.isNotEmpty
                                                      ? Image.network(
                                                          multipleImgsList[index],
                                                          fit: BoxFit.contain,
                                                        )
                                                      : const SizedBox());
                                            },
                                            itemCount: multipleImgsList.length),
                                      ),
                                      Visibility(
                                        visible: multipleImgsList.isNotEmpty,
                                        child: SmoothPageIndicator(
                                          controller: controller,
                                          count: multipleImgsList.length,
                                          effect: SlideEffect(
                                              dotHeight: 12,
                                              dotWidth: 12,
                                              dotColor: ColorsUtils.greyButton,
                    ...
                    CustomTextFormField(
                      ...
                      keyboardType: TextInputType.number,
                      onchanged: (val) {
                        return onChangeSlotRatesPerHour(val) ?? '';
                      },
                    ),
                    bookingType != 'G'
                        ? Column(
  ...
  Future getMultiplePicFromGallery() async {
    ...
    if (facilityImgIds!.length + pickedFile.length > 3) {
                                              ...
                                              if (finalSlotTime! > 1320) {
                                                ...
                                                if (finalSlotTime! > 0) {
                                                  ...
                                                  if (endTimeMinutes <= 1320) {
                                                    ...
                                                    showSnackBar(
                                                      'Slot must be end on the same day',
                                                      context,
                                                    );
                                                  }
                                                }
                ...
                GestureDetector(
                  onTap: () {
                    if (mAddFacilitySlotModel.slotsModelList!.length > 24) {
                      showSnackBar(
                        'Can not add more slots',
                        context,
                      );
                    } else {
                      if (finalSlotTime! >= 1320) {
                        showSnackBar(
                          'Slots full for the same day',
                          context,
                        );
                      } else {
  ...
  void checkForValidation() {
    ...
    if (selectedSubActivityID!.isEmpty) {
      showSnackBar(
        'Please select Activity & Sub-Activity',
        context,
      );
      ...
      showSnackBar(
        'Slot time can not less than 60 minutes',
        context,
      );
    } else if (_slotTimeInMins > 720) {
      showSnackBar(
        'Slot time must be less than 12 hours',
        context,
      );
    } else if (descriptionController.text.toString().trim().isEmpty) {
      showSnackBar(
      ...
      showSnackBar(
        'Please Add Listing Image',
        context,
      );
    } else if (facilityImgIds!.isEmpty) {
      showSnackBar(
        'Please add at least One Facility Image',
        context,
      );
    ...
    } else if (_isSameRatesForSlots && priceController.text.toString().trim().isNotEmpty && (double.parse(priceController.text) < 1)) {
      showSnackBar(
        'Rate/hour should be greater than or equals to 1',
        context,
      );
    } else if (personController.text.toString().trim().isEmpty) {
      showSnackBar(
        'Facility Capacity required',
        context,
      );
    } else if (personController.text.toString().trim().isNotEmpty && (double.parse(personController.text) < 1)) {
      ...
      if (isPriceChanged || isGroupSizeChanged || isCapacityChanged || isBookingTypeChanged || isSlotsChanged || isSlotTimeChanged) {
        showModalBottomSheet(
            context: context,
            isDismissible: false,
            enableDrag: false,
    ...
    //       msg: "Please select Activity & Sub-Activity",
    //       toastLength: Toast.LENGTH_SHORT,
    //       fontSize: 16.0);
    // } else if (_slotTimeInMins == 0) {
    //   Fluttertoast.showToast(
    //       msg: "Please add Slot Time",
    //       toastLength: Toast.LENGTH_SHORT,
    //       fontSize: 16.0);
    // } else if (addFacilitySlotList!.isEmpty) {
    //   Fluttertoast.showToast(
    //       msg: "Please Add Slots",
    //       toastLength: Toast.LENGTH_SHORT,
    //       fontSize: 16.0);
    // } else if (listingImgId == 0) {
    //   Fluttertoast.showToast(
    //       msg: "Please Add Listing Image",
    //       toastLength: Toast.LENGTH_SHORT,
    //       fontSize: 16.0);
    // } else if (facilityImgIds!.isEmpty) {
    //   Fluttertoast.showToast(
    //       msg: "Please add atleast One Facility Image",
    //       toastLength: Toast.LENGTH_SHORT,
    //       fontSize: 16.0);
    // } else if (_slotTimeInMins < 60) {
    //   Fluttertoast.showToast(
    //       msg: "60 mins default slot", toastLength: Toast.LENGTH_SHORT);
    // } else {
    //   setupApiCall();
    // }
  }
  ...
  void updateBasicInfo() async {
    Map request = {};
    request['FacilitySetupId'] = widget.getFacilityByIdModel!.facilitySetupId;
    ...
    request['ListingPageImageId'] = listingImgId!;
    ...
    try {
      await _progressDialog.show();
      String response = await Provider.of<ServiceProviderSetupViewModel>(context, listen: false).updateFacilityBasicInfo(request);
      debugPrint('callAddFacilitySetup -> $response');
      await _progressDialog.hide();
      if (response.isNotEmpty) {
        showSnackBarColor('Facility updated successfully', context, false);
  ...
  void setupApiCall() {
    ...
    addFacility['FacilitySetupId'] = widget.getFacilityByIdModel!.facilitySetupId;
    ...
    addFacility['RatePerHour'] = _isSameRatesForSlots ? double.parse(priceController.text.trim().isEmpty ? "0" : priceController.text.trim()) : null;
    addFacility['FacilityCapacity'] = personController.text.toString().trim();
  ...
  Future<void> callAddFacilitySetup(Map addFacility) async {
    try {
      await _progressDialog.show();
      String response = await Provider.of<ServiceProviderSetupViewModel>(context, listen: false).editFacilitySetupCall(addFacility);
      await _progressDialog.hide();
      if (response.isNotEmpty) {
        showSnackBarColor('Facility updated successfully', context, false);
        Navigator.of(context).pop(response);
  ...
  onChangeSlotRatesPerHour(String val) {
