...
class FacilityAddPageTwo extends StatefulWidget {
  ...
  const FacilityAddPageTwo(this.commonPassingArgs, {super.key});

  @override
  _FacilityAddPageTwoState createState() => _FacilityAddPageTwoState(commonPassingArgs);
}

class _FacilityAddPageTwoState extends State<FacilityAddPageTwo> {
  ...
  _FacilityAddPageTwoState(this._commonPassingArgs);
                  ...
                  children: [
                    ...
                    GestureDetector(
                      onTap: () async {
                        ...
                        _commonPassingArgs.endUserActivitySelection = interestValue;
                        var data = await Navigator.pushNamed(context, Constants.activityInterestFilterScreen, arguments: _commonPassingArgs);
  ...
  //   await _progressDialog.show();
  //   setState(() {});
  //   final pickedFile = await picker.pickMultiImage(imageQuality: 60);
  ...
  //           final byte = await File(pickedFile[i].path).length();
  //           final kb = byte / 1024;
  //           final mb = kb / 1024;
  //           debugPrint("File size -> ${mb.toString()}");
  //           if (mb > 10.0) {
  ...
  Future<void> facilityRegistration(Color textColor) async {
    if (selectedFilterData!.isEmpty) {
      ...
      await _progressDialog.show();
      Map facilityProviderRequest = {};
      facilityProviderRequest['FacilityProviderId'] = null;
      facilityProviderRequest['FacilityName'] = _commonPassingArgs.facilityName;
      facilityProviderRequest['CityId'] = countryID;
      facilityProviderRequest['PinCode'] = _commonPassingArgs.postalCode;
      facilityProviderRequest['Email'] = _commonPassingArgs.emilID;
      facilityProviderRequest['MobCountryCode'] = countryCode;
      facilityProviderRequest['MobileNumber'] = _commonPassingArgs.mobileNo;
      facilityProviderRequest['Address'] = _commonPassingArgs.address;
      facilityProviderRequest['UENRegistrationNo'] = _commonPassingArgs.registrationNumberUEN;
      facilityProviderRequest['EstablishmentYear'] = establishmentController.text.toString().trim();
      if (_commonPassingArgs.profileImageUploadId != null) {
        facilityProviderRequest['ProfileImageId'] = _commonPassingArgs.profileImageUploadId;
      } else {
        facilityProviderRequest['ProfileImageId'] = null;
      }
      facilityProviderRequest['OtherDescription'] = description.text.toString().trim();
      facilityProviderRequest['RegServiceProviderId'] = _commonPassingArgs.tempRegisterId;

      //new fields
      facilityProviderRequest['PayoutMethod'] = selectedPayoutMethod!.value;
      facilityProviderRequest['MobileNum'] = selectedPayoutMethod == PayoutMethodTypeEnum.payNow
          ...
          : selectedPayoutMethod == PayoutMethodTypeEnum.payLah
              ...
              : "";
      facilityProviderRequest['PayNowId'] = payNowIdController.text.trim();
      facilityProviderRequest['BeneficiaryName'] = beneficiaryNameController.text.trim();
      facilityProviderRequest['BankName'] = bankNameController.text.trim();
      facilityProviderRequest['BankAccountNumber'] = bankAccountNumberController.text.trim();
      facilityProviderRequest['IFSCSwiftCode'] = ifscCodeController.text.trim();
      facilityProviderRequest['IsTermsAccepted'] = termsAndCondition;

      facilityProviderRequest['USERID'] = null;
      facilityProviderRequest['Others'] = otherTextController.text.toString().trim();
      facilityProviderRequest['ReferrerCode'] = referralCode.text.toString().trim();
      List<Map> passingMap = [];
      for (var element in _commonPassingArgs.FacilityProviderContactDtos!) {
        Map map = {};
        map['Name'] = element.Name;
        map['MobileNumber'] = element.MobileNumber;
        map['MobCountryCode'] = element.MobCountryCode;
        passingMap.add(map);
      }
      debugPrint(passingMap.toString());
      facilityProviderRequest['FacilityProviderContactDtos'] = passingMap;
      ...
      facilityProviderRequest['FacilityProviderCertificationDtos'] = certificateIdMap;
      ...
      facilityProviderRequest['FacilityProviderSubActivityDtos'] = subActivityMapList;
      facilityProviderRequest['FacilityProviderPrvReviewDtos'] = [];
      facilityProviderRequest['FacilityProviderCancelPolicyDto'] = null;
      debugPrint('Request -> $facilityProviderRequest');

      try {
        await Provider.of<EndUserRegistrationViewModel>(context, listen: false).facilityProviderRegister(facilityProviderRequest).then(
          (value) async {
            Response res = value;
            if (res.statusCode == 500 || res.statusCode == 404) {
              await _progressDialog.hide();
              ...
              debugPrint("facility reg response -> $response");
