...
class CoachCancellationRequestPage extends StatefulWidget {
  const CoachCancellationRequestPage({super.key});

  @override
  State<CoachCancellationRequestPage> createState() => _CoachCancellationRequestPageState();
}

class _CoachCancellationRequestPageState extends State<CoachCancellationRequestPage> with TickerProviderStateMixin {
  ...
  Widget cancellationRequestListTile(CancellationRequest cancellationRequest) {
    ...
    //DateTime tempDate = DateFormat("yyyy-MM-dd'T'hh:mm:ss").parse(cancellationRequest.bookingDate.toString());
          ...
          child: SizedBox(
            ...
            child: ElevatedButton(
              ...
              onPressed: selectedRequestVerificationId != null
                  ? () {
                      updateCoachBookingRefundStatus(verificationId: selectedRequestVerificationId, accepted: false);
                    }
                  : null,
        ...
        Expanded(
          child: SizedBox(
            ...
            child: ElevatedButton(
              ...
              onPressed: selectedRequestVerificationId != null
                  ? () {
                      updateCoachBookingRefundStatus(verificationId: selectedRequestVerificationId, accepted: true);
                    }
                  : null,
              child: CustomTextView(
                label: 'Accept',
  ...
  Future<void> getTransaction(String coachId, bool clearList) async {
    setState(() {
      isLoading = true;
      if (clearList) {
    ...
    try {
      String requestStr = 'CoachId=$coachId&Page&PageStart=$pageCount&ResultPerPage=$resultPerPage';
      CancellationRequestsListModel cancellationRequestListResponse =
          await Provider.of<ServiceProvidersCancellationViewModel>(context, listen: false).getCoachTransactionList(requestStr);
      // if (!mounted) return;
  ...
  Future<void> updateCoachBookingRefundStatus({int? verificationId, required bool accepted}) async {
    await _progressDialog.show();
    Map request = {"BookingRefundVerificationId": verificationId, "Status": accepted ? "A" : "R"};
    try {
      var response = await Provider.of<ServiceProvidersCancellationViewModel>(context, listen: false).updateCoachBookingRefundStatus(request);
      if (!mounted) return;
      if (response.isNotEmpty) {
        showSnackBarColor('Request ${accepted ? "accepted" : "rejected"} successfully', context, false);
        Future.delayed(const Duration(seconds: 1), () async {
          await _progressDialog.hide();
          await getTransaction(OQDOApplication.instance.coachID!, true);
        });
      } else {
        showSnackBarErrorColor('We\'re unable to connect to server. Please contact administrator or try after some time', context, true);
      }
