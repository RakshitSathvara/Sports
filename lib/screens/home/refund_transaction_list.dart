...
import 'package:oqdo_mobile_app/model/refund_list_response.dart';
...
class RefundTransactionList extends StatefulWidget {
  const RefundTransactionList({Key? key}) : super(key: key);

  @override
  _RefundTransactionListState createState() => _RefundTransactionListState();
}

class _RefundTransactionListState extends State<RefundTransactionList> with AutomaticKeepAliveClientMixin<RefundTransactionList> {
                  ...
                  children: [
                    ListView.separated(
                      ...
                      itemBuilder: (BuildContext context, int index) {
                        DateTime tempDate = DateFormat("yyyy-MM-dd'T'hh:mm:ss").parse(transactions[index].refundCreatedAt.toString());
                        DateTime bookDate = DateFormat("yyyy-MM-dd'T'hh:mm:ss").parse(transactions[index].bookingDate.toString());
                        ...
                        String time = DateFormat.jm().format(DateTime.parse(transactions[index].refundCreatedAt.toString()));
                            ...
                            children: [
                              Row(
                                ...
                                children: [
                                  ...
                                  Text(
                                    '\$ ${transactions[index].refundAmount}',
                                    maxLines: 3,
                                    style: TextStyle(fontSize: 15, color: Theme.of(context).extension<CustomColors>()!.redAmount, fontWeight: FontWeight.w600),
                                    overflow: TextOverflow.ellipsis,
                                  ),
                                ],
                              ),
  ...
  Future<void> getTransaction() async {
    ...
    try {
      ...
      await Provider.of<ProfileViewModel>(context, listen: false).getRefundList(requestStr).then(
        (value) async {
          Response res = value;
          if (res.statusCode == 500 || res.statusCode == 404) {
            ...
            if (!mounted) return;
            RefundListResponse notificationResponse = RefundListResponse.fromJson(jsonDecode(res.body));
            if (notificationResponse.data != null && notificationResponse.data!.isNotEmpty) {
              setState(() {
                isLoading = false;
                transactions.addAll(notificationResponse.data!);
