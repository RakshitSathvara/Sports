...
class AddTrainingAddressPage extends StatefulWidget {
  String? type;
  EndUserAddressResponseModel? model;

  AddTrainingAddressPage({Key? key, this.type, this.model}) : super(key: key);

  @override
  AddTrainingAddressPageState createState() => AddTrainingAddressPageState();
}

class AddTrainingAddressPageState extends State<AddTrainingAddressPage> {
    ...
    try {
      ...
      if (getAllActivityAndSubActivityResponse.Data!.isNotEmpty) {
        ...
        if (widget.model != null) {
          ...
          getSubActivity();
          selectedSubActivityID = widget.model?.subActivityId.toString();
          choosediActivity = widget.model?.subActivitiesName ?? "";
          titleController.text = widget.model?.addressName ?? "";
          addressController.text = widget.model?.address1 ?? "";
          address2Controller.text = widget.model?.address2 ?? "";
          postalcode.text = widget.model?.pinCode ?? "";
  ...
  void callAddAddress() {
    ...
    addAddressMap['PinCode'] = postalcode.text.toString().trim();
    addAddressMap['CoachTrainingAddressId'] = widget.model?.endUserTrainingAddressId;
  ...
  void callEndUserAddress(BuildContext context) {
    ...
    endUserMap['EndUserTrainingAddressId'] = widget.model?.endUserTrainingAddressId;
  ...
  void callEndUserApiCall(addEndUserMap, BuildContext buildContext) async {
    try {
      ...
      await progressDialog.hide().then((value) {
