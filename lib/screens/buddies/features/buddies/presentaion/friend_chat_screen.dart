...
class FriendChatScreen extends StatefulWidget {
  ...
  const FriendChatScreen({Key? key, this.model}) : super(key: key);
            ...
            children: [
              Consumer<ChatProvider>(
                ...
                builder: (context, provider, child) {
                  if (isSending != provider.isSending) {
                  ...
                  if (provider.receivedFriendMessage != null) {
                    var msg = provider.receivedFriendMessage;
                    provider.resetNewMessage();

                    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
                      if (mounted) {
                        if (msg?.isSentFromMe == true) {
                          _enterMessageController.text = "";
                        }

                        if (widget.model!.fromEndUserId == msg!.fromEndUserId || widget.model!.fromEndUserId == msg.toEndUserId) {
                          if (widget.model!.toEndUserId == msg.fromEndUserId || widget.model!.toEndUserId == msg.toEndUserId) {
                            setState(() {
                              isEmpty = false;
                              messageList.insert(0, msg);
                              widget.model!.lastMessage = msg.message.toString();
                            });
                          }
                        }
                      }
                      ...
                      child: ClipPath(
                        ...
                        child: Opacity(
                          ...
                          child: Container(
                            ...
                            child: widget.model!.profileImage != null && widget.model!.profileImage!.isNotEmpty
                                ? CachedNetworkImage(
                              ...
                              ? GestureDetector(
                                  onTap: () async {
                                    if (_enterMessageController.text.toString().trim().isEmpty) {
                                      ...
                                      context.read<ChatProvider>().sendNewMsgToFriend(<Object>[
